"""Baseline schema — captures the hand-designed schema.sql (v1.2) as the
starting Alembic revision.

Previously this schema was applied by mounting db/schema.sql into Postgres's
docker-entrypoint-initdb.d (runs once, automatically, only on an empty data
volume) — that gave no version history, no way to review a diff before
applying a change, and no upgrade/downgrade path. From here on,
`alembic upgrade head` is the one supported way to apply or evolve the
schema; future changes are new revisions layered on top of this one (see
README's "Database migrations" section).

The SQL below is verbatim from db/schema.sql at the time this baseline was
captured — nothing was hand-converted to op.create_table() calls, since a
55KB / 53-table schema with generated columns, custom CHECK constraints, and
cross-table triggers is far more reliably reproduced by running the actual
DDL than by re-deriving it through SQLAlchemy's table-builder API.
...
def downgrade() -> None:
    # Full teardown — drops everything this baseline created. Deliberately
    # blunt (schema DROP CASCADE) rather than reverse-engineering 53
    # individual DROP TABLE statements in dependency order.
    #
    # Recreating an empty alembic_version table afterward matters: Alembic
    # itself writes to that table right after downgrade() returns (to record
    # that revision 0001 is no longer applied), and DROP SCHEMA CASCADE would
    # otherwise take that table down with everything else, causing Alembic's
    # own bookkeeping step to fail.
    op.execute("""
